            ...
            <div class="fichecontact">
                ...
                <?php
                    ...
                    if(isset($_GET['search']) AND !empty($_GET['search'])) {
                        ...
                        $articles = $connection->query('SELECT NumArt FROM article WHERE LibTitrA LIKE "%'.$search.'%" ORDER BY NumArt DESC');
                        if($articles->rowCount() == 0) {
                            $articles = $connection->query('SELECT NumArt FROM article WHERE LibChapoA LIKE "%'.$search.'%" ORDER BY NumArt DESC');
                        }
                ...
                while($rows = $articles->fetch()) {
                    $titre=$rows['NumArt'];
                    $ContArt = "SELECT * FROM article WHERE NumArt='$titre'";
                    $result = $connection->query($ContArt);
                    if ($result) {
                        while ($tuple = $result->fetch()) {
                ?>
                        <div class="traitsepare"></div>
                        <p class="titreartrech"><?= "<a style='text-decoration:none; color:#F8F8F8;' href=''>".$tuple["LibTitrA"]."</a>" ?></p>
                        <p class="datemotcle"><?=$tuple["DtCreA"]?></p>
                        <div class="rowrech">
                            <div class="imgrech" style="background-image:url(<?=$tuple["UrlPhotA"]?>);"></div>
