<?php
  ...
  $sql = 'SELECT * FROM motcle WHERE NumMoCle=:id'; // Tout sélectionner à propos de la table langue ou NumLang=:id
  ...
  if (isset ($_POST['Lbc']) ) { // Test des informations du form
    ...
    $sql = 'UPDATE motcle SET LibMoCle=:Lbc WHERE NumMoCle=:id'; // Mise en place de l'Update
    $statement = $connection->prepare($sql); // Préparation de sql
...
<div class="container">
  <div class="card mt-5">
    <div class="card-header">
      <h2>Mettre à jour la langue : <?= $person->LibMoCle; ?> ? </h2> <!-- Récupération de Lib1Lang et affichage -->
    </div>
    <div class="card-body">
      ...
      <form method="post">
        <div class="form-group">
          <label for="Lbc">Libellé :</label>
          <input value="<?= $person->LibMoCle; ?>" type="text" name="Lbc" id="Lbc" class="form-control" maxlength="30" required> <!-- Récupération de la vateur Lib1Lang, définition d'un maxlength et force à ce que l'utilisateur remplisse la case -->
        </div>
        <div class="form-group">
