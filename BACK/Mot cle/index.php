...
$sql = 'SELECT * FROM motcle'; // Met dans la varaible toute la sélection de la table langue
...
<div class="container"> <!-- Code bootstrap -->
  <div class="card mt-5 bkcard"> <!-- Code bootstrap -->
    <div class="card-header"> <!-- Code bootstrap -->
      <div class="box">
        <div class="gauche" ><h2>Mot clef  :</h2></div><!-- Titre h2 -->
    ...
    <div class="card-body"> <!-- Code bootstrap -->
      <table class="table table-bordered"> <!-- Code bootstrap -->
        ...
        <?php foreach($langues as $row): ?> <!-- Tableau en PHP -->
          <tr>
            <td><?= $row->LibMoCle; ?></td> <!-- Lecture ligne par ligne -->
            <td><?= $row->NumLang; ?></td>
            <td>
            <?php
              $id = $row->NumMoCle ; // Rentre dans $id la combinaison de $row->NumLang
              $Lib1Lang = $row->LibMoCle;
            ?>
